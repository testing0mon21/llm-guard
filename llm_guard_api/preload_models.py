        ...
        for scanner_config in scanners:
            ...
            if scanner_type:
                ...
                try:
                    ...
                    _ = input_util.get_scanner_by_name(scanner_type, scanner_params)
    ...
    try:
        ...
        for scanner_config in scanners:
            ...
            if scanner_type:
                ...
                try:
                    # Инициализируем сканер только для загрузки моделей
                    _ = output_util.get_scanner_by_name(scanner_type, scanner_params)
                    logger.info(f"Модели для сканера {scanner_type} успешно загружены")
                except Exception as e:
                    logger.warning(f"Не удалось загрузить модели для сканера {scanner_type}: {e}")
    except ImportError as e:
...
def preload_vision_models() -> None:
    """Предварительная загрузка PaddleOCR (en, ru) c onnxruntime и YOLO ONNX (если путь задан)"""
    try:
        from app.vision import initialize_vision_models

        yolo_path = os.environ.get("YOLO_ONNX_PATH", "")
        yolo_conf = float(os.environ.get("YOLO_CONF", "0.25"))
        initialize_vision_models(languages=["en", "ru"], yolo_onnx_path=yolo_path, yolo_confidence_threshold=yolo_conf)
        logging.info("Vision models preloaded (PaddleOCR en/ru, YOLO if provided)")
    except Exception as e:
        logging.warning(f"Не удалось предзагрузить vision модели: {e}")
